...
public class StudyWithController {
	...
	@RequestMapping(value = "/with/studylist")
	...
	@RequestMapping("/with/studydetail")
	...
	@RequestMapping("/with/teacher/studydetail")
	...
	@RequestMapping(value = "/with/teacher/studyinsert") // 강사 마이페이지에서 넘겨야함
	public String insertForm() {
		return "studyinsert_with";
	}

	@RequestMapping(value = "/with/teacher/studyinsertres", method = RequestMethod.POST)
	...
	@RequestMapping("/with/teacher/studyupdate")
	public String studyUpdate(Model model, int class_no) {
		model.addAttribute("studyDto", studyBiz.selectOne(class_no));
		return "studyupdate";
	}

	@RequestMapping("/with/teacher/studyupdateres")
	public String studyUpdateres(StudyDto dto) {

		int res = studyBiz.update(dto);

		if (res > 0) {
			return "redirect: with/studydetail?class_no=" + dto.getClass_no();
		}
		return "redirect: with/studyupdate?class_no=" + dto.getClass_no();
	}

	@RequestMapping("/with/teacher/studydelete")
	...
	// 음 alert 처리?
	@RequestMapping("/with/teacher/imagedelete") //댓글 삭제  
    @ResponseBody
