package com.kh.onsu.qna.controller;
...
import com.kh.onsu.qna.model.biz.QnaBiz;
...
public class QnaController {
	...
	public String selectOne(Model model, Principal principal, @RequestParam("qnano") int qnano) {
		...
		logger.info("username : " + member_id);
	...
	@RequestMapping(value = "/qnainsertres")
	public String insertRes(Model model, QnaDto qnaDto) {
		logger.info("QnaController.insertForm - qnawriter : " + qnaDto.getQnawriter());
		...
		if (res > 0) {
			...
			model.addAttribute("url", "/qna/qna");
			return "redirect";
		} else {
	...
	@RequestMapping(value = "/qnasetsecret")
	public String setSecret(Model model, QnaDto qnaDto) {
		...
		if (res > 0) {
			...
			return "qnaone?qnano=" + qnaDto.getQnano();	
		}
	}
	...
	@RequestMapping(value = "/qnaupdateres")
	public String updateRes(QnaDto qnaDto) {
		...
		if (res > 0) {
			return "redirect: qnaone?qnano=" + qnaDto.getQnano();
		} else {
			return "redirect: qnaupdate?qnano=" + qnaDto.getQnano();	
		}
	}
	...
	@RequestMapping(value = "/qnaanswerres")
	public String answerRes(QnaDto qnaDto) {
		...
		if (res > 0) {
			...
			return "redirect: qnaanswer?qnano=" + qnaDto.getQnano();	
		}
	}
