...
if (!document.getElementById('nickname')) {
	...
	name2 = document.getElementById('nickname').value;
}
...
STUN:	Session Traversal Utilities for NAT의 약어이며, 
	  	상대방 클라이언트에 이 정보를 전달해 주며 이를 통해 P2P 연결을 성립시키는 역할

TURN: 	Traversal Using Relay around NAT의 약어이고, 
	  	STURN 서버를 이용한 연결이 실패했을 경우에 사용
		공인 IP로 돌아가는 TURN 서버를 이용해
		각 클라이언트가 보내는 데이터를 다른 클라이언트에게 보내주는 작업을 실시하여 P2P 연결을 성립시켜 줌
	
		즉, STUN/TURN 서버는 중계자 역할과 비슷
	
Signaling: 	서로 다른 네트워크에 있는 Client들은 서로를 연결하기 위해서
			각 클라이언트의 위치와 미디어 포맷 협의가 필요한데,
			이러한 프로세스를 진행해주는 게 Signaling

Signaling 서버 : 두 클라이언트 사이에서 서로의 Signaling 정보를 전달 할 수 있는 서버가 필요하며
				어떠한 통신 방법이든 상관 없음
				Signaling 서버는 클라이언트에서 오는 정보를 다른 클라이언트로 전달하는 역할만 수행
*/
...
// 클라이언트에게 오디오를 활성화할지, 비디오를 활성화할지 제안
...
if (maxClient) {
	...
	connection.maxParticipantsAllowed = prompt("최대 인원을 설정하세요");
	while (isNaN(connection.maxParticipantsAllowed)) {
		connection.maxParticipantsAllowed = prompt("최대 인원을 다시 설정하세요");
	}
		console.log("최대 인원 수 : " + connection.maxParticipantsAllowed);
	}
}
...
connection.onstream = function(event) {
	...
	if (event.type == 'local') {
		...
		while (name1 == null || name1 == '' || name1 == ' ' || name1 == undefined) {
			event.extra.fullName1 = prompt("이름을 입력하세요");
			if (event.extra.fullName1 != null || event.extra.fullName1 != '' || event.extra.fullName1 != ' ' || event.extra.fullName != undefined) {
				name1 = event.extra.fullName1;
				false;
			}
		}
		h2.innerHTML = event.extra.fullName1;
	...
	if (event.type == 'remote') {
		...
		name2 = event.extra.fullName2;
		h2.innerHTML = event.extra.fullName2;
		// h2.HTML 값을 위에 connection.extra의 fullName 값을 넣음
...
openbutton.onclick = function() {
	...
	if (roomid.value == null || roomid.value == '') {
		...
		// 일정 시간마다 반복해주는 setInterval 함수 단위는 mill
		setInterval(function(){
		
			var clnum = document.querySelector('#clientnumber');
				connection.getAllParticipants().forEach(function(participantId) {
				    var user = connection.peers[participantId];
					var hisFullName  = user.extra.fullName1;
					var hisUID = user.userid;
					
				});
				 var numberOfUsers = connection.getAllParticipants().length;
					clnum.innerHTML = "최대 인원 수 : " + (connection.maxParticipantsAllowed-1) + "<br>현재 시청자 : " + numberOfUsers + "명";
				},5000);
	}
		...
		connection.join(roomid.value, function(isRoomJoined, roomid, error){
			...
			setInterval(function(){
				connection.getAllParticipants().forEach(function(participantId) {
					...
					var clnum = document.querySelector('#clientnumber');
					var numberOfUsers = connection.getAllParticipants().length;
					clnum.innerHTML = "현재 시청자 : " + numberOfUsers + "명";
				},1000);
			});
		}
