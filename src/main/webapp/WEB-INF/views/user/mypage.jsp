...
%>
	...
	<section>
		...
		<div class="mypageBox">
			<div class="ldiv">
				<div class="myp_subtitle">${mlist.member_id }님의 강의 일정</div>
				<div class="cdiv">
					<table class="ldiv" id="cal">
						...
						<tr>
							...
							%>
							<td>
								<a class="countView" href="calendarListView.do?year=<%=year%>&month=<%=month%>&date=<%=i%>&member_id=${mlist.member_id }" style="color:<%=Utils.fontColor(i, dayOfWeek)%>">
									<%=i%>
								</a> 
					...
					<table class="list">
						...
						<c:choose>
							<c:when test="${empty lwlist && empty lvlist }">
								<tr>
									<td colspan="2">수강 중인 강의가 없습니다.</td>
								</tr>
							</c:when>
							<c:otherwise>
						...
						<tr>
							<c:choose>
								<c:when test="${empty callist }">
									<tr>
										<td colspan="2" style="font-weight: bold;">수업 일정이 없습니다.</td>
									</tr>
								</c:when>
								<c:otherwise>
									<c:forEach items="${callist }" var="calendardto">
										<tr>
											<td style="text-align:left;">
												<a style="color: blue;padding-left:20px;overflow:auto;" href="calendarDetail.do?calendar_no=${calendardto.calendar_no }">${calendardto.calendar_title }</a>
											</td>
											<td>
